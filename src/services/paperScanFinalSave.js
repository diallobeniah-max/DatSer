...
// The months a sheet applies to. Accepts BOTH the modern `months` array and the
// legacy single `month` key so existing plan callers and saved scans keep working.
const attendanceMonthsForSettings = (settings) => {
  const months = Array.isArray(settings?.months) && settings.months.length
    ? settings.months
...
const collectAttendanceItems = ({ row, settings }) => {
  const items = []
  let unresolved = 0
  const months = attendanceMonthsForSettings(settings)
  if (!months.length) return { items, unresolved }
  months.forEach((month) => {
    const selected = selectedSundaysForMonth(settings, month)
  ...
  sheetList.forEach((sheet) => {
    ...
    result.payload.rows.forEach((row, rowIndex) => {
      ...
      if (isCreateNewRow(row)) {
        ...
        // A new member is created in every month this sheet applies to, so its
        // attendance in each selected month can be written. Falls back to the
        // explicit target month when no month settings exist yet.
...
// Final creation authorization must use fresh server state, never the
// review-start snapshot. If a fresh snapshot provider is supplied it is awaited;
// otherwise the provided currentMembers list is used as the fallback.
export const resolveMembersForDuplicateCheck = async ({ currentMembers = [], fetchFreshMembers = null }) => {
  if (typeof fetchFreshMembers === 'function') {
    const fresh = await fetchFreshMembers()
    if (Array.isArray(fresh)) return fresh
  }
  return Array.isArray(currentMembers) ? currentMembers : []
}
...
// A server-side duplicate conflict is not an ordinary step failure: the client
// must surface the candidate for operator resolution, not just mark the row failed.
export const isDurableDuplicateConflict = (error) => (
  Boolean(error?.blockedDuplicate) || Boolean(error?.duplicateCandidate)
)

export const duplicateConflictFromError = (error) => ({
  sheetId: null,
  rowIndex: null,
  name: error?.duplicateCandidate?.full_name || error?.duplicateCandidate?.name || 'Unnamed',
  matches: error?.duplicateCandidate ? [error.duplicateCandidate] : [],
  level: 'server-recheck',
  candidate: error?.duplicateCandidate || null
})
...
const executeDurableStep = async ({ operationId, stepId, supabase, action }) => {
  ...
  if (data?.success !== true) {
    if (data?.blocked_duplicate || data?.duplicate_candidate) {
      const duplicateCandidate = data?.duplicate_candidate || null
      const error = new Error(data?.error_message || `${action} found a possible existing member before save.`)
      error.blockedDuplicate = true
      error.duplicateCandidate = duplicateCandidate || null
      throw error
    }
    throw new Error(data?.error_message || `${action} could not be completed.`)
  }
  return data
}
...
export const executeFinalSave = async ({ plan, confirmedDuplicateKeys = [], deps = {} }) => {
  const {
    ...
    operationId = null,
    savedScanId = null,
    // A fresh authoritative member snapshot provider. When supplied, duplicate
    // detection runs against THIS (server-current) list rather than the possibly
    // stale review-start snapshot. Falls back to currentMembers when absent.
    fetchFreshMembers = null
  } = deps
  if (!supabase) {
  ...
  const memberIds = {}
  const membersForDuplicateCheck = await resolveMembersForDuplicateCheck({ currentMembers, fetchFreshMembers })
  ...
  for (const entry of plan.rows) {
    ...
    try {
      ...
      if (isDurableDuplicateConflict(error)) {
        // Server-side recheck found an existing active candidate at the moment of
        // INSERT. Do not count as a plain failure: surface for operator review.
        const duplicate = duplicateConflictFromError(error)
        duplicate.sheetId = entry.sheetId
        duplicate.rowIndex = entry.rowIndex
        ...
        members.push({
          ...
          attendanceUpdated,
          skippedUnresolved: 0,
          reason: error?.message || 'Possible existing member found before save. Review before creating a new record.'
        })
        continue
      }
      summary.failed += 1
...
export const finalSaveResultFromOperation = (operation) => {
  ...
  const members = rows.map((row, index) => {
    ...
    const duplicateBlock = failed?.result?.blocked_duplicate || failed?.result?.duplicate_candidate
    ...
    const candidate = failed?.result?.duplicate_candidate || null
    if (duplicateBlock) {
      blocked.push({
    ...
    return {
      ...
      status: duplicateBlock
        ? FINAL_SAVE_STATUS.BLOCKED_DUPLICATE
        : failed || incomplete
...
export const retryPersistedFinalSave = async ({ operationId, deps = {} }) => {
  ...
  for (const step of operation.steps || []) {
    ...
    // A server-confirmed duplicate is an operator-resolution state, not a
    // transient failure. Re-executing it would re-attempt the INSERT; leave it
    // blocked so the operator resolves it (use existing / confirm distinct).
    const alreadyBlocked = step?.result?.blocked_duplicate || step?.result?.duplicate_candidate
