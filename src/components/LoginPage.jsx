...
const LoginPage = () => {
  ...
  const [authType, setAuthType] = useState('user') // 'user' | 'admin'
    ...
    <div className="min-h-screen relative overflow-hidden flex items-center justify-center p-4" style={authPageStyle}>
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-white/95 dark:bg-gray-900/85 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-6 backdrop-blur-sm">
          {/* Mode Header with Top-Right User / Admin Selector */}
          <div className="flex items-start justify-between gap-3 mb-5">
            {(mode === 'forgot' || mode === 'magiclink') && authType === 'user' && (
              <button
                type="button"
                onClick={() => switchMode('login')}
                className="p-1 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 -ml-1"
              >
            ...
            {/* Compact Two-Mode Selector at Top-Right */}
            <div className="inline-flex rounded-xl bg-stone-100 p-1 dark:bg-stone-800 border border-stone-200/80 dark:border-stone-700/80 shrink-0" role="tablist" aria-label="Login mode">
              <button
                type="button"
                role="tab"
                aria-selected={authType === 'user'}
                onClick={() => {
                  setAuthType('user')
                  setError(null)
                }}
                className={`rounded-lg px-2.5 py-1 text-xs font-bold transition-all ${
                  authType === 'user'
                    ? 'bg-white text-stone-900 shadow-xs dark:bg-stone-700 dark:text-white font-black'
          ...
          {authType === 'admin' ? (
            /* Admin Mode Form */
            <form onSubmit={handleAdminCodeSignIn} className="space-y-4">
              <div className="space-y-1.5">
                <label className="block text-left text-xs font-bold uppercase tracking-wider text-stone-500 dark:text-stone-400">
                  Admin Code
                </label>
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" />
              ...
              <button
                ...
                className="w-full py-3 rounded-xl font-medium text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm"
              >
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === 'Enter' && !isLoading) {
                            e.preventDefault()
                            if (e.currentTarget.form?.requestSubmit) {
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' && !isLoading) {
                          ...
                          if (e.currentTarget.form?.requestSubmit) {
                            ...
                            const submitFn = mode === 'login' ? handleEmailSignIn : mode === 'signup' ? handleEmailSignUp : mode === 'magiclink' ? handleMagicLinkSignIn : handleForgotPassword
                            submitFn(e)
                          }
                        }
                      }}
                        ...
                        onKeyDown={(e) => {
                          if (e.key === 'Enter' && !isLoading) {
                            ...
                            if (e.currentTarget.form?.requestSubmit) {
                              ...
                              const submitFn = mode === 'login' ? handleEmailSignIn : handleEmailSignUp
                              submitFn(e)
                            }
                          }
                        }}
