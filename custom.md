...
## Physical iPhone Dock Rule
- Bottom Search, Scan, and Add controls must remain fully visible above the safe area.
- No ancestor may clip the floating dock.
- Keyboard geometry changes must be immediate and must not animate through intermediate positions.
- The top navigation remains anchored and only the central member list scrolls.
- The blur remains behind the controls in closed and keyboard-open states.

## Member Code Format Rule
- Member-code format is workspace-wide and shared by collaborators.
- Supported formats are Letters + Numbers and Letters Only.
- Letters-only codes use a deterministic unique alphabetical sequence.
- Code changes must be transactional and concurrency-safe.
- Search, QR, scanner, badges, exports, caches, and forms must use the same current format.
- Exact code searches return the exact member instead of the full list.
- Existing printed codes should remain resolvable through safe aliases where supported.
