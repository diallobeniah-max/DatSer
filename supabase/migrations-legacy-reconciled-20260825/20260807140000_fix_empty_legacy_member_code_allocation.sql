-- Fix: historical member transfer fails when the member has no existing
-- workspace_member_codes assignment.
--
-- set_member_attendance_from_other_month imports a source member then calls
-- ensure_workspace_member_code(p_owner_id, jsonb_build_object('id', member_id))
-- (no legacy_code supplied). ensure_workspace_member_codes normalised the empty
-- legacy_code to '' and INSERTed it, violating:
--   workspace_member_codes_legacy_code_check = legacy_code ~ '^[A-Z0-9]+$'
--
-- Corrective behaviour (constraint is preserved, not weakened):
--   * when a legacy_code is supplied and non-empty, it is preserved exactly
--     (and kept as an alias when it differs from the generated candidate);
--   * when legacy_code is null/empty/whitespace, a valid legacy_code is derived
--     from the already-generated candidate/current_code (always [A-Z0-9]+), so
--     the INSERT never writes an empty legacy_code;
--   * existing allocations are untouched (the `if exists ... continue` guard and
--     the `on conflict (workspace_owner_id, member_id) do nothing` are kept);
--   * collision handling via the per-prefix suffix scan is unchanged.
...
begin
  ...
  loop
    ...
    -- Members without a supplied legacy_code still receive a valid,
    -- constraint-satisfying legacy_code derived from the generated candidate.
