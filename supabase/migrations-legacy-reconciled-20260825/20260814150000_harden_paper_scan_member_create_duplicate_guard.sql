...
--
-- PREPARED ONLY: do not apply from this review. Additive migration that hardens
-- the already-applied `20260813170000_harden_paper_scan_final_save` durable
-- member-create path WITHOUT modifying that historical migration.
--
...
-- ---------------------------------------------------------------------------
-- Server-safe normalization helpers (immutable, no external extensions).
-- ---------------------------------------------------------------------------
...
returns text language sql immutable set search_path = pg_catalog, public, pg_temp as $$
  select lower(regexp_replace(lower(coalesce(p_value, '')), '[^a-z0-9]+', ' ', 'g'))
$$;
...
-- Identity-scoped serialization key. Phone when present, otherwise normalized
-- name. Two distinct people sharing a family phone briefly serialize (acceptable);
-- they are never auto-merged because the candidate query below requires BOTH a
-- normalized phone AND a normalized name match.
...
-- ---------------------------------------------------------------------------
-- Hardened member-create step. Same signature/authorization/idempotency as the
-- applied 1700 version, plus the fresh duplicate guard before INSERT.
-- ---------------------------------------------------------------------------
...
begin
  ...
  begin
    ...
    if s.kind = 'member-create' then
      ...
      -- jsonb_array_elements_text yields text rows already; aggregate directly.
