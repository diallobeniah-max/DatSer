-- Paper Scan Saved Scans: atomic staging metadata. PREPARED ONLY: do not
-- apply until Codex gates it.
--
-- The old staging path rebuilt the whole `sheet_images` array on the client
-- from a shared in-memory Map. When two uploads completed concurrently, the
-- task that finished LAST could write a stale array and silently drop the
-- other sheet's durable reference (A reads v1, B reads v1, B writes A+B, A
-- finishes later and writes A-only -> B disappears).
--
-- This migration replaces that with a single SECURITY DEFINER RPC that:
--   - requires a permanent (non-anonymous) authenticated user,
--   - authorizes the workspace via the existing
--     public.require_permanent_workspace_actor() helper,
--   - locks the target Saved Scan row FOR UPDATE so concurrent merges
--     serialize on the DATABASE, not on client state,
--   - reads the LATEST sheet_images inside the lock,
--   - merges exactly ONE sheet (add or update by sheetId),
--   - preserves every other existing sheet reference,
--   - writes the merged state and returns the durable row.
--
-- Client completion order therefore cannot lose a successfully persisted
-- sheet: every merge is one atomic read-merge-write under the row lock.
--
-- The companion remove RPC deletes a sheet reference from the durable
-- metadata only. It never touches the remote storage object; destructive
-- object deletion remains a separate, confirmed "Delete Saved Scan" action.
