-- Paper Scan — additive profile-field persistence for durable save steps.
--
-- Prepared for review. Do not deploy without explicit authorization.
--
-- The applied `20260815134507_harden_paper_scan_member_create_duplicate_guard`
-- function already persists Full Name, Phone Number, Gender, and Current Level,
-- but silently drops Age and Parent/Guardian fields. The Paper Scan review UI
-- allows editing those fields, so this migration extends the SAME function to
-- persist them safely without modifying the historical migration sources.
--
-- Rules honoured here:
--   * Additive create-or-replace only. No destructive table drops, deletes, or
--     table alterations.
--   * Never touches attendance, member codes, provenance, or unrelated tables.
--   * Preserves the 1700 signature/authorization/idempotency and the 1415
--     duplicate-guard semantics exactly.
--   * Every optional column is checked with `month_table_has_column` before it
--     is written, so legacy month tables without a column are never corrupted.
--   * Values are inlined with `quote_nullable` (NULL-safe literal quoting); the
--     member id and workspace owner stay parameterized as $1 / $2.
