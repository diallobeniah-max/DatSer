-- Migration: Cross-Month Member Search and Safe Attendance/Import RPCs (Hardened)
-- Provides safe, transactional, RLS-isolated cross-month search and dynamic single-member attendance import.

-- 1. RPC: search_workspace_members_across_months
-- Securely searches authorized monthly tables belonging to p_owner_id (excluding p_current_table).
-- Validates current table ownership and excludes soft-deleted source members.
-- Deduplicates results ONLY by canonical member ID (uuid).
...
begin
  ...
  if p_current_table is not null and p_current_table <> '' then
    select exists (
      ...
      where user_id = p_owner_id and table_name = p_current_table
  ...
  for v_rec in (
    ...
    if not exists (
      ...
      where table_schema = 'public' and table_name = v_rec.table_name
    ) then
      continue;
    end if;
...
-- 2. RPC: set_member_attendance_from_other_month
-- Dynamic, transactional, idempotent single-member import/restore & Present/Absent attendance marking.
-- Validates date belongs to target month, requires active source record, and dynamically intersects columns.
...
begin
  ...
  select exists (
    select 1 from public.user_month_tables
    where user_id = p_owner_id and table_name = p_source_table
  ) into v_source_valid;
  ...
  if v_reserved_request_id is null then
    ...
    where owner_id = p_owner_id
      ...
      and operation_name = 'set_member_attendance_from_other_month'
  ...
  begin
    ...
    -- Compute column intersection between source and target tables
    ...
    v_sql := format(
      'select exists (select 1 from %I where id = $1) as ex',
      p_target_table
    );
    ...
    if v_target_exists then
      if v_has_deleted then
        execute format('select exists (select 1 from %I where id = $1 and deleted_at is not null)', p_target_table)
