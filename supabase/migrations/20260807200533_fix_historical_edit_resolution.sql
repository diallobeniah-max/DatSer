...
begin
  ...
  if not exists (
    ...
    where umt.user_id = p_owner_id
      and umt.table_name = p_table_name
  ) then
    raise exception 'Table % is not part of this workspace', p_table_name;
  end if;

  execute format(
    'select array_agg(id), count(*) from %I where id = $1 and deleted_at is null',
    p_table_name
  ) into v_ids, v_count using p_member_id;
  ...
  if v_phone is not null and v_phone_column is not null then
    execute format(
      'select array_agg(id), count(*) from %I where deleted_at is null and lower(btrim(%I)) = lower($1) and btrim(coalesce(%I, '''')) = $2',
      p_table_name,
      v_name_column,
      v_phone_column
    ) into v_ids, v_count using v_name, v_phone;
  else
    execute format(
      'select array_agg(id), count(*) from %I where deleted_at is null and lower(btrim(%I)) = lower($1)',
      p_table_name,
      v_name_column
    ) into v_ids, v_count using v_name;
  end if;
